...
public class jsoupLogin {
	...
	public void login(String userName, String pwd) throws Exception {
		...
		// 第一次请求
		Connection con = Jsoup.connect("http://weibo.com/login");// 获取连接
		con.header("User-Agent","Mozilla/5.0 (Windows NT 6.1; WOW64; rv:29.0) Gecko/20100101 Firefox/29.0");// 配置模拟浏览器
		Response rs = con.execute();// 获取响应
		Document d1 = Jsoup.parse(rs.body());// 转换为Dom树
		List<Element> et = d1.select(".W_login_form");// 获取form表单，可以通过查看页面源码代码得知

		// 获取，cooking和表单属性，下面map存放post时的数据
		Map<String, String> datas = new HashMap<>();
		for (Element e : et.get(0).getAllElements()) {
			if (e.attr("node-type").equals("username_box")) {
				e.child(0).child(0).attr("value", "haoweiguoyouxiang@163.com");// 设置用户名
			}

			if (e.attr("node-type").equals("password_box")) {
				e.child(0).child(0).attr("value", "haoweiguo1"); // 设置用户密码
			}
